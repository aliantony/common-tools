...
public class AssetSoftwareServiceImpl extends BaseServiceImpl<AssetSoftware> implements IAssetSoftwareService {
            ...
            public Integer doInTransaction(TransactionStatus transactionStatus) {
                try {
                    ...
                    stream.forEach(s -> {
                        ...
                        BusinessExceptionUtils.isTrue(sysId.contains(s), "兼容系统不存在，或已经注销！");
                    ...
                    assetOperationRecord.setContent("登记资产");
                    ...
                    BusinessExceptionUtils.isTrue(!StringUtils.equals("品类型号不存在，或已经注销", e.getMessage()),
                        "品类型号不存在，或已经注销");
                    BusinessExceptionUtils.isTrue(!StringUtils.equals("兼容系统不存在，或已经注销！", e.getMessage()),
                        "兼容系统不存在，或已经注销！");
                    logger.error("录入失败", e);
                }
                return 0;
                ...
                if (!CollectionUtils.isEmpty(categoryOsResponseList)) {
                    ...
                    for (String osSystemId : osSystemIds) {
                        ...
                        if (optional.isPresent() && optional.get() != null) {
                            osSystemNameList.add(optional.get().getName() != null ? optional.get().getName() : "");
                        }
                    }
    ...
    public ActionResponse softwareInstallConfig(ConfigRegisterRequest request) throws Exception {
        ...
        query.setAssetId(request.getAssetId());
        query.setSoftwareId(request.getSoftwareId());
        Integer installStatus = assetSoftwareRelationDao.findInstallStatus(query);
        if (SoftInstallStatus.CONFIGURING.getCode().equals(installStatus)) {
...
//                assetSoftwareRelation.setInstallTime(softwareReportRequest.getInstallTime());
    ...
    public void querySoftwareLicense(SoftwareQuery softwareQuery,
        ...
        assetSoftwareDetailResponse.setSoftwareLicense(null);
    }
