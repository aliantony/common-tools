...
public class AssetServiceImpl extends BaseServiceImpl<Asset> implements IAssetService {
    ...
    public AssetOuterResponse getByAssetId(QueryCondition condition) throws Exception {
        ...
        if (StringUtils.isNotBlank(asset.getCustomField())) {
            List<AssetCustomizeRequest> assetCustomizeRequests = JSONArray.parseObject(asset.getCustomField(),
                new TypeReference<List<AssetCustomizeRequest>>() {
                });
            assetResponse.setCustomField(assetCustomizeRequests);
        }
        // 件
        //        List<AssetAssemblyRequest> assetAssemblyList = assetAssemblyDao.findAssemblyByAssetId(condition.getPrimaryKey(),
        //            null);
        //        // 查询软件
        //        List<AssetSoftware> assetSoftwareList = assetSoftwareRelationDao
        //            .getSoftByAssetId(DataTypeUtils.stringToInteger(condition.getPrimaryKey()));查询组
