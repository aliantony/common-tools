...
public class AssetLendRelationResponse extends BaseResponse {
    ...
    public String getOtherInfo() {
        ...
        if(lendStatus==1 && lendPeriods !=null && lendTime!=null){
          return String.format("借用人：%s;借用目的：%s;状态：%s;借用时间：%4$tY/%4$tm/%4$td-%5$tY/%5$tm/%5$td;归还时间：",useName,lendPurpose==null?"":lendPurpose,getLendStatusDesc(),new Date(lendTime),new Date(lendPeriods));
        }
        else if(lendStatus==2 &&  returnTime !=null && lendPeriods !=null && lendTime!=null)
            return String.format("借用人：%s;借用目的：%s;状态：%s;借用时间：%4$tY/%4$tm/%4$td-%5$tY/%5$tm/%5$td;归还时间：%6$tY/%6$tm/%6$td %6$tH:%6$tM",useName,lendPurpose==null?"":lendPurpose,getLendStatusDesc(),new Date(lendTime),new Date(lendPeriods),new Date(returnTime));
        else return null;
    }
